    ...
    return (
      <div style={{ margin: '8% 9%' }}>
        <div style={{ marginBottom: '80px' }}>
          ...
          <div className={styles.statBoxContainer}>{this.renderStatBoxes()}</div>
        </div>
        <div style={{ marginBottom: '80px' }}>
          <h5 style={{ fontSize: '22px' }}>Project Data</h5>
          <div className={`progress ${styles.bar}`}>
            <div
              className={`progress ${styles.barProgress}`}
              style={{ width: `${this.state.project.progress}%` }}
            ></div>
          </div>
        </div>
        <div style={{ marginBottom: '20px' }}>{this.renderAbouts()}</div>
      </div>
    )
  }
}
